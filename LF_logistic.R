...
LF_logistic<-function(X,y,loading,weight=NULL,intercept=TRUE,init.Lasso=NULL,lambda=NULL,mu=NULL,step=NULL,resol = 1.5,maxiter=10){
  ...
  else
  {
    ...
    if(count!=0 && intercept==TRUE)
    {
      ...
      debias.est=expo(lasso.plugin+correction*loading.norm)
      rhoh <-debias.est*(1-debias.est)
      se<-rhoh*sqrt(mean((Xc%*%direction)^2*weight^2*deriv.vec))*loading.norm/sqrt(n) #is this correct?
